...
const QuestionRenderer = ({ question, selected = {}, setSelected }) => {
          ...
          <table className="w-full max-w-lg border border-gray-300 border-collapse text-sm bg-white shadow-lg rounded-lg overflow-hidden">
            <thead>
              <tr className="bg-gradient-to-r from-blue-500 to-blue-600 text-white">
                {Object.keys(question.tableHeadings || {}).map(
                  (colKey, idx) => (
                    <th
                      key={idx}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div>
            ...
            {question.instructions && (
              <div
                className="bg-blue-10 border-l-4 border-blue-500 text-gray-800 p-3 sm:p-4 rounded-xl shadow-sm text-sm sm:text-base mb-2"
                dangerouslySetInnerHTML={{ __html: question.instructions }}
              />
            )}

            {/* Table Section */}
            <div className="overflow-x-auto rounded-lg border border-gray-200 shadow-sm w-11/12 sm:w-4/5 mx-0">
  ...
  return (
    <div className="text-center text-gray-500">
      Question ID {question.id} not designed yet.
    </div>
  );
};
